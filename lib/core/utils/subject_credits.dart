...
class SubjectCredits {
  ...
  /// Preloads subject credits from local cache, and optionally syncs from Supabase if online
  static Future<void> init() async {
    try {
      // 1. Try to load from SQLite cache first
      final cachedStr = await DatabaseHelper.instance.getCache('subject_credits');
      if (cachedStr != null) {
        final Map<String, dynamic> decoded = json.decode(cachedStr);
        _loadedCredits = decoded.map((key, value) => MapEntry(key, (value as num).toDouble()));
        debugPrint('Loaded ${_loadedCredits.length} subject credits from SQLite cache.');
      }
      ...
      if (isOnline) {
        ...
        for (var row in response) {
          ...
          if (code.isNotEmpty) {
            fetched[code.toUpperCase().trim()] = credits;
          }
        }

        if (fetched.isNotEmpty) {
          _loadedCredits = fetched;
          // Save to SQLite cache
          await DatabaseHelper.instance.saveCache('subject_credits', json.encode(fetched));
          debugPrint('Synced ${fetched.length} subject credits from Supabase.');
        }
      }
    } catch (e) {
      debugPrint('Error initializing SubjectCredits: $e');
    }
  }
  ...
  static double getCredits(String code) {
    ...
    // Dynamic heuristic fallbacks for unknown courses
    if (cleanCode.contains('LAB') || 
      ...
      if (cleanCode.contains('PROJECT')) {
        return 2.0; // Default project
      }
    }
    
    return 3.0; // Default theory credit
  }
